...
def fail_filters(record, dict_para, info: {}, format_values: {}, nb_alt: int, AD: [], filter_ad: int = 1, filter_qual=50, filter_mbq: int = 20, filter_mqsbz = 0.5, filter_dp: int = 10, filter_popaf: float = 0.0001, unpaired: bool = False):
	'''
	...
	if dict_para['vcf_annotation_method'].upper() == 'FUNCOTATOR':
		...
		elif dict_para['sequencing'].upper() == 'WES' and filter_on_mutation_position:
	...
	ad1 = float(AD[0])
	ad2 = float(AD[1])
	if float(ad1) < float(filter_ad) and float(ad2) < float(filter_ad):
		fail[i].add('AD')
	...
	max_vaf_sample = float(dict_para['VAF_sample_max_threshold'])
...
def create_new_records(dict_para, record, strFieldSplit, intIndex, InfoFieldsToProcess, strCRLF, filter_ad=1, filter_mbq=20, filter_dp=10, filter_qual=50, filter_mqsbz=0.5,filter_popaf=0.00001, unpaired: bool = False):
	...
	fail = fail_filters(record, dict_para, recInfo, format_values, len(record[intIndex['Alt']].split(',')), recAD, filter_ad, filter_qual, filter_mbq, filter_mqsbz, filter_dp, filter_popaf, unpaired) #False if the variant does not pass the filters and the name of the filter that does not pass otherwise
...
def filter(dict_colors, dict_para, output1, output2, vcf_file: str, logger: str, working_method : str, filter_ad: int, filter_qual: int, filter_mbq: int, filter_dp: int,
	...
	with open(vcf_file, mode='r', encoding='latin-1', errors='backslashreplace') as obFileInput:		 #read in latin-1 because some char from funcotator anotation are not in utf-8
		...
		if working_method.upper() == 'INMEMORY':
			...
			with tqdm(total=n - (n_header - intCountHeaderIns), disable=no_tqdm_bar, bar_format='{l_bar}{bar:30}{r_bar}', ncols=140, smoothing=1) as pbar:
				...
				pbar.close()
			if dict_para['colored_execution'].upper() == 'TRUE' or dict_para['colored_execution'].upper() == 'YES':
					print(f'\033[1m{color}{passed_count}\033[0m{color} variants have passed the filter!')
			else:
...
def main(dict_colors, args, vcf_file, recover):
	...
	output_path = dict_para['output_path_sample'].split('.vcf')[0] + '/'
	...
	logger.info(f'** cmd line : python lotus.py filter -v {str(vcf_file)} -o {output1} -wm {working_method} --DP {filter_dp} --MBQ {filter_mbq} '+str(no_argument)+' **')
