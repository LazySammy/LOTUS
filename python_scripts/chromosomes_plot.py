...
def get_value_for_chr(position, chr, length, span, step):
	...
	positions = [i for i in range(0, int(int(span) / 2), step)] + positions
...
def create_plot(args,cytobands_infos, curves_infos_b1, curves_infos_b2, curves_infos_sample, chromosomes_size, counts, plot_size, plot_name, title = '', logger = None):
	...
	if "SVG" in args['chromosomes_plot_format(s)'].upper():
		plot_name = Path(plot_name).with_suffix('.svg')
		plt.savefig(plot_name, format='svg', bbox_inches='tight')
	elif "PNG" in args['chromosomes_plot_format(s)'].upper():
		plot_name = Path(plot_name).with_suffix('.png')
		plt.savefig(plot_name, format='png', bbox_inches='tight')
	elif "JPG" in args['chromosomes_plot_format(s)'].upper():
		plot_name = Path(plot_name).with_suffix('.jpg')
...
def create_chromosomes_plot(args,gene_positions_b1 : dict, gene_positions_b2 : dict, gene_positions_sample : dict, cytoband_file : str, plot_name : str, step_on : int = 500000, logger = None) :

	span_on = int(step_on)*2
	...
	create_plot(args, cytobands_infos, curves_infos_b1, curves_infos_b2, curves_infos_sample, chromosomes_size, counts, plot_size, plot_name, '', logger)