...
def create_dataframe_from_gene(dict_para, d):
	...
	col = ['Number of variants', 'Details (snp, dnp, tnp, onp, ins, del)', 'Ref', 'Alt variant(s)', 'Chromosome',
		   'Position(s)', 'DP', 'QUALITY', 'Mutation subtypes']  # Columns name
...
def create_snp_plot(dict_para, d, dcount, name, vcf_name, logger, output_path):
	...
	df["Count"] = counts
	df.rename(columns=lambda x: re.sub(r'.*passed.*', 'Frequency (%)', x), inplace=True)
	...
	if "TSV" in dict_para['SNP_profile'].upper():
...
def count_variants(dict_parameters, dict_colors, vcf_file_filter, vcf_file_pass, genome, logger):
	...
	if vcf_file_filter:
		...
		with tqdm(total=nb_lines, disable=no_tqdm_bar, bar_format='{l_bar}{bar:30}{r_bar}', ncols=150, smoothing=1) as pbar:
...
def create_mutations_types_barplot(dict_para):
	...
	plt.title("Mutation counts, for each mutation type \nfound in " + sample_name, pad=10)
