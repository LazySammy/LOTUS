...
def manage_parameters(config_file, recover):
	...
	if output_folder.upper() == 'FALSE':
		...
		with open("input/config.txt", 'w') as output_file:
			for line in filtered_lines:
				if not line.startswith("#"):
					cleaned_line = line.replace('\n', '')
				elif 'GLOBAL' in line.upper():
		...
		if dict_parameters['module(s)'].replace(" ", "").upper() != 'COMPARE,MERGE' and dict_parameters['module(s)'].replace(" ", "").upper() != 'COMPARE':
			input_vcf = dict_parameters['vcf_folder_path']
			if not input_vcf.endswith('/'):
				dict_parameters['vcf_folder_path'] = input_vcf + '/'
				input_vcf += '/'
			...
			with open(input_vcf + vcf_files[0], 'r') as file:
				for line in enumerate(file):
					if 'base_qual,Description="alt m' in str(line) and dict_parameters['vcf_annotation_method'].upper() != 'FUNCOTATOR':
						...
						sys.exit(
							"It seems that you have vcf files annotated by FUNCOTATOR, but you didn't put FUNCOTATOR as vcf_annotation_method. Please change it in the config.txt file.")
					elif '##INFO=<ID=INDEL,Number=0,Type=Flag,Description="Indicates that the variant is an INDEL.">' in line and dict_parameters[
						'vcf_annotation_method'].upper() != 'ANNOVAR':
						# dict_parameters['vcf_annotation_method'] = 'ANNOVAR'
						sys.exit(
							"It seems that you have vcf files annotated by ANNOVAR, but you didn't put ANOVAR as vcf_annotation_method. Please change it in the config.txt file.")
		dict_parameters['output_path'] = output_path
