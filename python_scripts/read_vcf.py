...
def read_vcf(vcf_file, get_variant_idx=False):
	...
	with open(vcf_file, 'r', encoding='latin-1') as vcf:
		if not get_variant_idx:
			for line in vcf:
				if line != '':
					if line[0] != '#':  # if line is not a header
						yield (line.strip().split('\t'))  # add the line as a list of strings in the object
					elif line.startswith('#') and not line.startswith('##'):  # if we are on the header line
						...
						yield (idx)  # add the index as a dictionnary in the object
		else:
			pos = vcf.tell()
			line = vcf.readline()
