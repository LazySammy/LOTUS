...
def extract_biological_process(dict_para, list_genes: list, toppgene_name: str, panther_name: str, logger=None, pvalue=0.05, maxres=100):
	if len(list_genes) == 0:
		if logger:
		...
		cURL = r"""curl -H 'Content-Type: text/json' -d '{"Genes":""" + str(genes) + r""", "Type": "GeneOntologyBiologicalProcess", "PValue": """ + str(
			pvalue) + r""", "MinGenes": 1, "MaxGenes": 1500, "MaxResults": """ + str(
			maxres) + r""", "Correction": "FDR"}' https://toppgene.cchmc.org/API/enrich > """ + ToppGeneEnrichment
		if dict_para['verbose_prints'].upper() == 'TRUE':
